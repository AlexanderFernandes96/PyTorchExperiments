...
if strcmp(channel_type, 'unstructured')
    ...
    if strcmp(pilot_scheme{1}(1:2), 'FD')
        Ga  = sqrt(1/2)*(randn(M,M) + 1j*randn(M,M)); % AP  -> AP
        Har = sqrt(1/2)*(randn(N,M) + 1j*randn(N,M)); % AP  -> RIS
    end
elseif strcmp(channel_type, 'geometric')
    ...
    if strcmp(pilot_scheme{1}(1:2), 'FD')
        Ga = sqrt(1/2)*(randn(M,M) + 1j*randn(M,M));      % AP  -> AP
        Har = generateLOSchannel([Nw,Nh],2, M,1, LOS);   % AP  -> RIS
    end
end
...
% Har = sqrt(PL(g0, d_AP_RIS, a_AP_RIS))*Har;
...
if strcmp(pilot_scheme{1}(1:2), 'FD')
    HarHra_kr = khatrirao(Har.', Hra);
end

% Vectorization of all channels
if strcmp(pilot_scheme{1}(1:2), 'FD')
    h = [Ga(:); HarHra_kr(:); Hua(:); HurHra_kr(:)];
elseif strcmp(pilot_scheme{1}(1:2), 'HD')
    h = [Hua(:); HurHra_kr(:)];
end
